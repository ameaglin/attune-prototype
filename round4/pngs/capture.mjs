...
const BASE = 'http://127.0.0.1:8766/round4';
...
await shot('03-paid-intro-3-app-guide');
await evalp(() => toggleObGuide('circle'));
await shot('03b-paid-intro-3-circle-open');
...
await evalp(() => closeExerciseFab());
await evalp(() => goTab('home'));
await shot('53d-home-s4-prep');
await evalp(() => startPrepFlow({ toChecklist: true }));
await shot('53e-s4-prep-checklist');
await evalp(() => closePrepOverlay());
...
await evalp(() => renderDgeWarmup());
await shot('56b-dge-warmup');
await evalp(() => startDgeWarmupPlay());
await shot('56c-dge-warmup-play');
await evalp(() => renderDgeThemes());
await shot('59-dge-themes');
await evalp(() => openDgeTheme('trust'));
await shot('59b-dge-theme-detail');
await evalp(() => renderDgeListen());
await shot('59c-dge-listen');
await evalp(() => renderDgeEdges());
await shot('60-dge-edges');
await evalp(() => continueDgeEdges());
await shot('60b-dge-ghost');
...
await evalp(() => showScreen('ob-intro-3'));
await shot('71b-free-intro-3');
await evalp(() => toggleObGuide('circle'));
await shot('71c-free-intro-3-circle');
await evalp(() => showScreen('ob-path'));
